...
use app\admin\model\Sysmodule as SysmoduleModel;
use think\facade\Session;
use app\admin\model\authRule;


// 模型控制器
class Sysmodule extends Common{
    ...
    public function addrule($data){
           ...
           $authRule['title'] = $data["Database_detail"];
          ...
          if(!empty($authRule_id)){
           ...
           // 删除-----------
           $authRule['title'] = $data["Database_detail"].'删除';
           $authRule["href"] = $data['module'].'/del';
           $authRule["menustatus"] = 0;
           authRule::create($authRule);
           // 删除end-----------
    ...
    public function index(){
        if(request()->isPost()) {
           ...
           $list = $model->where($where)->order('sys_module_id desc')->paginate(array('list_rows' => $pageSize, 'page' => $page))->toArray();

            foreach ($list['data'] as $key => &$value) {
                // 列表页 循环处理条件 都放在这里index_for_list_data
                $value['sys_module_add'] = date('Y-m-d H:i:s',$value['sys_module_add']);
            }
    ...
    public  function edit($id=''){
        ...
        if(request()->isPost()){
            $data = input('post.');
            // 更新数据过滤掉非数据表字段数据
            if($model->allowField(true)->save($data,['sys_module_id' => $data['sys_module_id']])){
                $result['msg'] = '修改成功!';
     ...
     */
    public function Del(){
      if(SysmoduleModel::destroy(input('id'))){
    ...
    public function add(){
        if(request()->isPost()){
                ...
                // 保存数据表
                $SysmoduleModel_data['sys_module_name'] = $data['module'];
                $SysmoduleModel_data['sys_module_table'] = $data['Database'];
                $SysmoduleModel_data['sys_module_detail'] = $data['Database_detail'];
                $SysmoduleModel_data['sys_module_add'] = time();
                $SysmoduleModel_data['sys_module_uid'] = session('aid');
                $model = new SysmoduleModel($SysmoduleModel_data);
                // 过滤post数组中的非数据表字段数据
                ...
                // //---------------------- 创建模型-------------------------------
                  $this->create_file($this->modelPath.'/'.ucwords($data['module']).'.php');  //创建文件
                  $model_str = $this->read_file($this->model);   //读取文件
                  $model_str = str_replace('?{{module}}?', ucwords($data['module']), $model_str); // 替换
                ...
                //  // ---------------------- 生成控制器-------------------------------
                  $this->create_file($this->path.'/'.ucwords($data['module']).'.php');  //创建文件
                  $model_controller_str = $this->read_file($this->controller);   //读取文件
                  $model_controller_str = str_replace('?{{module}}?', ucwords($data['module']), $model_controller_str);  // 替换
                  ...
                  // 添加数据库
                  // $addtable_res =  $this->addtable($data); 
                  ...
                  foreach ($data['Database_en'] as $key => $value) {
                        switch ($data['Database_type'][$key]) {
                            ...
                            case '9': //普通文本
                                ...
                                $model_controller_str_select = str_replace('index_for_list_data', "index_for_list_data\n" . $select_index_radio_str, $model_controller_str_select);
                            ...
                            case '8': //下拉框
                                ...
                                $model_controller_str_select = $this->read_file($this->path.'/'.$data['module'].'.php');   //读取文件
                                  // 替换
                                 $pattern = $this-> tab('phpForeach',$view_demo,$key,$data);
                                 $model_controller_str_select = str_replace('//index_for_list_data', "//index_for_list_data\n".$pattern, $model_controller_str_select);
                                 // 替换
                                 $this->write_file($this->path.'/'.$data['module'].'.php',$model_controller_str_select);  //写入
                    ...
                    switch ($data['Database_type'][$key]) {
                      ...
                      case '2': //单选按钮
                        ...
                        case '9':
                             ...
                             $model_controller_str_select = $this->read_file($this->path.'/'.$data['module'].'.php');   //读取文件
                             $model_controller_str_select = str_replace('add_for_list_data', "add_for_list_data\n" . $select_add_date_str, $model_controller_str_select);
                             $this->write_file($this->path.'/'.$data['module'].'.php',$model_controller_str_select);  //写
                             // 直接修改控制器 end
            ...
            //  ---------------------- edit视图创建------------------------------
                  ...
                  $manyImgArray  = '';
                    ...
                    case '9':
                             ...
                             $model_controller_str_select = $this->read_file($this->path.'/'.$data['module'].'.php');   //读取文件
                             $model_controller_str_select = str_replace('edit_for_list_data', "edit_for_list_data\n" . $edit_for_list_data_str, $model_controller_str_select);
                             $this->write_file($this->path.'/'.$data['module'].'.php',$model_controller_str_select);  //写
                             // 直接修改控制器 end
                             $shijianchajian_js .= $this-> tab('view-add-date-js',$view_demo,$key,$data);
    ...
    public function updata(){
        ...
        $model = new SysmoduleModel();
        // 过滤post数组中的非数据表字段数据
        if($model->allowField(true)->save($data,['sys_module_id' => $data['id']])){
    ...
    public function tab($tab='',$view_demo,$key,$data)
    {
        ...
        $r_str = $this-> th(['?{{major}}?','?{{Database}}?','?{{module}}?','?{{Database_en}}?','?{{Database_zh}}?','?{{status}}?','?{{Database_en_array}}?'],[$data['major'],$data['Database'],$data['module'],$data['Database_en'][$key],$data['Database_zh'][$key],$data['Database_en'][$key],$data['xiala_data'][$key]],$result[1]);
        return $r_str;
